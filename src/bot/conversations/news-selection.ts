...
export async function newsSelectionConversation(
  ...
  try {
    ...
    if (!selectedNews || selectedNews.length === 0) {
      await ctx.reply("❌ Нет новостей для обработки. Сначала выполните команду /get_posts и дождитесь завершения обработки.");
      return;
    }
    ...
    if (isNaN(postIndex) || postIndex < 0 || postIndex >= allNews.length) {
      await ctx.reply(`❌ Неверный номер поста. Доступны посты от 1 до ${allNews.length}. Попробуйте еще раз.`);
      return;
    }
    ...
    const processedPost = await processNewsWithN8n(selectedPost, selectedChannel, userId);
...
async function displayNewsList(ctx: Context, news: any[]) {
  ...
  logger.debug({
    ...
    sampleTitles: news.slice(0, 3).map(item => item.title?.substring(0, 50) || 'no title')
  });
 ...
 */
async function processNewsWithN8n(post: any, selectedChannel?: any, userId?: number, retryCount: number = 0): Promise<any | null> {
  ...
  try {
    ...
    const requestData = {
      title: post.title,
      description: post.summary || post.description, // используем summary или description
      link: post.url || post.link, // используем url или link
      image: post.image,
    ...
    const processedPost = response.data;
    if (processedPost) {
 ...
 */
async function displayProcessedPost(ctx: Context, processedPost: any) {
