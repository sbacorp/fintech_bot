-- Supabase Schema для Telegram бота
...
-- Политики для user_sessions (доступ для сервиса - отключаем RLS для сервисного использования)
CREATE POLICY "Service can manage user sessions" ON user_sessions
    FOR ALL USING (true);

-- Политики для user_news (доступ для сервиса - отключаем RLS для сервисного использования)
CREATE POLICY "Service can manage user news" ON user_news
    FOR ALL USING (true);

-- Политики для processed_posts (доступ для сервиса - отключаем RLS для сервисного использования)
CREATE POLICY "Service can manage processed posts" ON processed_posts
    FOR ALL USING (true);

-- Политики для post_stats (доступ для сервиса - отключаем RLS для сервисного использования)
CREATE POLICY "Service can manage post stats" ON post_stats
    FOR ALL USING (true);

-- Политики для grammy_sessions (доступ для сервиса)
CREATE POLICY "Service can manage grammy sessions" ON grammy_sessions
    FOR ALL USING (true);
